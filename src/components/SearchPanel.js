...
import { SearchInput, Input, Button } from './Style'
import SectionCenter from "./SectionCenter";

function SearchPanel() {

  const [text, setText] = useState({user: 0}) 
  const [user, setUser] = useState('')

  const [load, setload] = useState(false) 

  const [effe,setEffe] = useState(false) 
  ...
  return (
    <>
      <SearchInput>
        ...
        <Input type="text" placeholder='Search GitHub username...' value={user} onChange={inpSearch} />
        <Button type='button' onClick={click} >Search</Button>
      </SearchInput>
      {/* {
         load ? <SectionCenter data={text} /> : <h1>Loading</h1>  
       } */}
       <SectionCenter data={text}    />
    </>
  )
}
